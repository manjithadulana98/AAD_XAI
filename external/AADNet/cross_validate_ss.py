    ...
    for s in range(from_sbj, to_sbj):
        ...
        if model_params['model_name'] in ['LSQ', 'CCA']:
            print(f'{datetime.now().time().replace(microsecond=0)} --- '
                    f'********** training Sbj {s} **********')
            train_accs[:,s,:], test_accs[:,s,:] = parallelizeLSQ(config, crossSSData, nFold, s, parallelization=running_params['parallelization'])
        else:
            for fold in range(nFold):
                print(f'{datetime.now().time().replace(microsecond=0)} --- '
                    f'********** training Sbj {s} - Fold {fold} **********')                                  
                # update lr from tuned hyperparameters for NSR model
                if model_params['model_name']=='EEGNetAAD':
                    out_dir = os.path.abspath(setup_params['output_path'])
                    dataset_name = config.get(('dataset', 'name'))
                    json_file = os.path.join(out_dir, 'trained_models', f'{dataset_name}_cnn_train_params_P{s:02d}_fold_{fold}.json')
                    if os.path.exists(json_file):
                        lr = json.load(open(json_file, 'r'))['lr']
                # model
                ...
                # dataset
                ((eegs_tr, envs_tr, y_tr),(eegs_te, envs_te, y_te)) = crossSSData[fold]
                ...
                model_path = os.path.join(output_path, f"{model_params['model_name']}_SS_T_{T}_s_{s}_fold_{fold}.pth")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='Subject-specific cross validation.')
    ...
    save_path = os.path.join(output_path, f"{jobname}_SS_acc.npy")    
    ...
    for i in range(len(configs)):
        print(configs[i])
        (_, te_acc, _, _) = trainSS(configs[i], jobname)
        SS_test_accs.append(te_acc)
    SS_test_accs = np.array(SS_test_accs)
    #
    if os.path.isfile(save_path):
        all_SS_test_accs = np.load(save_path)
    else:
        num_window = len(configs[0].get(('dataset', 'windows')))
        num_sbjs = len(configs[0].get(('dataset', 'all_sbjs')))
        all_SS_test_accs = np.zeros((len(configs), num_window, num_sbjs))
    all_SS_test_accs[:,:,from_sbj:to_sbj] = SS_test_accs
    np.save(save_path, all_SS_test_accs)
    print(f'all_SS_test_accs: {all_SS_test_accs}')
