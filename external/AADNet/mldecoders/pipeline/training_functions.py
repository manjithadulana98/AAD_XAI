...
import torch
import os
from pipeline.datasets import HugoMapped, OctaveMapped, MiscellaneousDS
...
def train_dnn(data_dir,
    ...
    if dataset=='hugo':
        ...
        train_dataset = MiscellaneousDS(train_parts, data, participant=participants, num_input=mdl.input_length)
        train_loader = DataLoader(train_dataset, batch_size=batch_size, sampler = torch.randperm(len(train_dataset)), num_workers=workers, pin_memory=True)
        val_dataset = MiscellaneousDS(val_parts, data, participant=participants, num_input=mdl.input_length)
        val_loader = DataLoader(val_dataset, batch_size=1250, sampler = torch.randperm(len(val_dataset)), num_workers=workers, pin_memory=True)        
    ...
    for epoch in range(epochs):
        ...
        print(f'Epoch {epoch}: accuracy = {mean_accuracy}')
