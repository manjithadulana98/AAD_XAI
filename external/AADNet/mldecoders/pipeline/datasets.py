...
class MiscellaneousDS(Dataset):
    '''
    dataloader for reading EventAAD, DTU, KULeuven dataset
    '''
    def __init__(
        self,
        parts_list,
        ...
        self.num_input=num_input
        self.channels = np.arange(pooledData[0][0][0].shape[0])
    ...
    def _initialise_data(self):
        ...
        stim = []
        for each_participant in self.participants:
            for i in self.parts_list:
                eeg += [self.pooledData[each_participant][0][i]]
                label = self.pooledData[each_participant][2][i]
                stim += [self.pooledData[each_participant][1][i][label]]

        self.eeg = np.concatenate(eeg,axis=1)
        self.stim = np.concatenate(stim)

    def __getitem__(self, idx):
        return self.eeg[:, idx:idx+self.num_input], self.stim[idx]
    
    def __len__(self):
        print(f'len: {self.stim.size - self.num_input}')
        return self.stim.size - self.num_input        